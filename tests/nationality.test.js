...
test(`renders player's VALID nationality as such if getNationalityHTML is INVALID (not a function)`, async () => {
  ...
  expect(consoleWarn.mock.calls[0][0]).toMatch(
    `Impossible value provided for "getNationalityHTML" option`,
  );
});
    ...
    contestants: {
      c1: {
        players: [
          {
            nationality:
              '<span class="user-nationality" style="width: 36px">US</span>',
          },
        ],
      },
    },
    ...
    matches: [
      {
        roundIndex: 0,
        order: 0,
        sides: [{ contestantId: "c1" }, { contestantId: "c2" }],
      },
    ],
    contestants: {
      c1: {
        players: [{ title: "Pete", nationality: "US" }, { title: "Ivan" }],
      },
      c2: {
        players: [
...
test(`renders bare player.nationality if getNationalityHTML throws`, () => {
  ...
  expect(wrapper.querySelector(".nationality").textContent).toBe("US");
  expect(consoleWarn.mock.calls[0][0]).toMatch(
    `Failed to get a string from getNationalityHTML`,
  );
});
...
test(`injects a valid return value of getNationalityHTML to ALL players`, () => {
  ...
  const nat_elements = [
    ...wrapper.querySelectorAll(".player-wrapper .nationality"),
  ];
  const all_players_have_foo = nat_elements.every(
    (el) => el.innerHTML === "foo",
  );
