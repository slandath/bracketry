...
test(`renders no player title if player.title is undefined`, () => {
  ...
  expect(
    wrapper.querySelector('.side-wrapper[contestant-id="c1"] .player-title')
      .textContent,
  ).toBe("");
});
...
test(`inserts HTML returned by getPlayerTitleHTML to the DOM`, () => {
  ...
  const { wrapper } = init(data, {
    getPlayerTitleHTML: (br) => `<a class="user-link">${br.title}</a>`,
  });
  expect(wrapper.querySelector(".player-title .user-link").textContent).toBe(
    "Pete",
  );
});
