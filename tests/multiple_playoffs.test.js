...
test(`uninstalls old bracket when new bracket is installed into the same wrapper`, () => {
  ...
  expect(wrapper.querySelectorAll(".side-wrapper[contestant-id]").length).toBe(
    0,
  );
});
...
test(`two coexisting brackets don't intervene in each other's "highlighted" state`, () => {
  ...
  w1.querySelector(benfica_side_selector).dispatchEvent(
    new MouseEvent("click", { bubbles: true }),
  );

  expect(
    w1.querySelectorAll(benfica_side_selector + ".highlighted").length,
  ).toBe(2);
  expect(
    w2.querySelectorAll(benfica_side_selector + ".highlighted").length,
  ).toBe(0);
});
