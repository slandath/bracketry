...
test(`renders empty .entry-status IF side has NO entryStatus AND getEntryStatusHTML is NOT provided`, () => {
  ...
  expect(
    wrapper.querySelector(".side-info-item.entry-status").textContent,
  ).toBe("");
});
...
test(`renders side's VALID entryStatus as such if getEntryStatusHTML is INVALID (not a function)`, async () => {
  ...
  expect(consoleWarn.mock.calls[0][0]).toMatch(
    'Impossible value provided for "getEntryStatusHTML" option',
  );
});
...
test(`renders html provided as entryStatus (when no options.getEntryStatusHTML)`, () => {
  const data = {
    ...
    contestants: {
      c1: {
        entryStatus:
          '<span class="user-status" style="background: tomato">WC</span>',
      },
    },
  };
  const { wrapper } = init(data);
...
test(`renders bare contestant.entryStatus if getEntryStatusHTML throws`, async () => {
  ...
  await new Promise((r) => setTimeout(r, 0));
  expect(consoleWarn.mock.calls[0][0]).toMatch(
    `Failed to get a string from getEntryStatusHTML`,
  );
});
  ...
  const data = {
    ...
    matches: [
      ...
      },
      {
        roundIndex: 0,
        order: 1,
        sides: [{ contestantId: "c3" }, { contestantId: "c4" }],
      },
    ],
