...
test(`highlights team history (add "highlighted" class to .match-wrapper and .side-wrapper)`, () => {
  ...
  expect(
    wrapper.querySelectorAll(benfica_selector + ".highlighted").length,
  ).toBe(0);

  wrapper
    .querySelector(benfica_selector)
...
test(`highlights next contestant's history after next click`, () => {
  ...
  wrapper
    .querySelector(real_selector)
    .dispatchEvent(new MouseEvent("click", { bubbles: true }));

  expect(wrapper.querySelectorAll(".match-wrapper.highlighted").length).toBe(4);
  expect(wrapper.querySelectorAll(real_selector + ".highlighted").length).toBe(
    4,
  );
  expect(
    wrapper.querySelectorAll(benfica_selector + ".highlighted").length,
...
test(`does not highlight when something other than '.side-wrapper[contestant-id]' is clicked`, () => {
  ...
  wrapper
    .querySelector(".matches-positioner")
    .dispatchEvent(new MouseEvent("click", { bubbles: true }));
...
test(`does not highlight when side-wrapper without [contestant-id] is clicked`, () => {
  ...
  wrapper
    .querySelector(".side-wrapper:not([contestant-id])")
    .dispatchEvent(new MouseEvent("click", { bubbles: true }));
  expect(wrapper.querySelectorAll(".match-wrapper.highlighted").length).toBe(0);
});
...
test(`does not unhighlight history after click outside .matches-positioner`, () => {
  ...
  wrapper
    .querySelector(".round-titles-wrapper")
    .dispatchEvent(new MouseEvent("click", { bubbles: true }));
...
test(`highlights a contestant history when highlightContestantHistory is called with a valid contestantId`, () => {
  ...
  expect(
    wrapper.querySelectorAll(
      `.side-wrapper[contestant-id='villarreal'].highlighted`,
    ).length,
  ).toBe(3);
});
...
test(`highlights a contestant history when highlightContestantHistory is called with a valid contestantId`, () => {
  ...
  const highlighted_side_wrappers = wrapper.querySelectorAll(
    `.side-wrapper.highlighted`,
  );
  expect(highlighted_side_wrappers.length).toBe(3);
  expect(highlighted_side_wrappers[0].getAttribute("contestant-id")).toBe(
    "villarreal",
  );
  expect(highlighted_side_wrappers[2].getAttribute("contestant-id")).toBe(
    "villarreal",
  );
});
...
test(`last highlighted match has a "last-highlighted" class (and there is only one such match)`, () => {
  ...
  const highlighted_match_wrappers = wrapper.querySelectorAll(
    `.match-wrapper.highlighted`,
  );
  const last_highlighted =
    highlighted_match_wrappers[highlighted_match_wrappers.length - 1];
  expect(last_highlighted.classList.contains("last-highlighted")).toBe(true);

  expect(
    wrapper.querySelectorAll(`.match-wrapper.last-highlighted`).length,
  ).toBe(1);
});
...
test(`does not highlight contestant's matches ON CLICK when options.onMatchClick is defined`, () => {
  ...
  expect(
    wrapper.querySelectorAll(
      `.side-wrapper[contestant-id='benfica'].highlighted`,
    ).length,
  ).toBe(0);
});
...
test(`can highlight contestant's matches on highlightContestantHistory() call
  ...
  const benfica_sides = wrapper.querySelectorAll(
    `.side-wrapper[contestant-id='benfica']`,
  );
  const highligted_benfica_sides = wrapper.querySelectorAll(
    `.side-wrapper[contestant-id='benfica'].highlighted`,
  );
...
spoilt_ucl.matches.forEach((m) => {
  // i need matches with some contestantIds missing
  m.sides.forEach((s) => {
...
test(`does not highlight anonymous (contestant-less) sides when a side without [contestant-id] is clicked`, () => {
  ...
  const anonymous_side_wrapper = wrapper.querySelector(
    ".side-wrapper:not([contestant-id])",
  );
  anonymous_side_wrapper.dispatchEvent(
...
test(`unhighlights when a side without [contestant-id] is clicked`, () => {
  ...
  const anonymous_side_wrappers = wrapper.querySelectorAll(
    ".side-wrapper:not([contestant-id])",
  );
  anonymous_side_wrappers[0].dispatchEvent(
    new MouseEvent("click", { bubbles: true }),
  );
  expect(wrapper.querySelectorAll(".side-wrapper.highlighted").length).toBe(0);
});
