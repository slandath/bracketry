...
test(`draws new data supplied via replaceData`, () => {
  ...
  expect(wrapper.querySelectorAll(".round-title")[0].textContent).toBe(
    "round 1",
  );
    ...
    matches: [
      {
        roundIndex: 0,
        order: 0,
        sides: [{ contestantId: "c1", scores: [{ mainScore: 1 }] }],
      },
    ],
...
test(`keeps an old data when replaceData is called with critically invalid data (and keeps DOM intact)`, () => {
  ...
  bracket.replaceData({
    rounds: [],
    matches: [{ roundIndex: 1, order: 1, sides: NaN }],
  });
  ...
  expect(
    wrapper
      ...
      .querySelector(".player-title").textContent,
  ).toBe("Villarreal");
});
...
test(`resets navigation state when calling replaceData`, () => {
  ...
  expect(
    getComputedStyle(wrapper.querySelectorAll(".round-wrapper")[0]).display,
  ).not.toBe("none");
});
