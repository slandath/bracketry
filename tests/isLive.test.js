...
test(`does not apply live styles to a match element which { isLive: false }`, () => {
  ...
  expect(
    getComputedStyle(wrapper.querySelector(".match-body")).borderColor,
  ).not.toBe("red");
  expect(
    getComputedStyle(wrapper.querySelector(".match-body")).backgroundColor,
  ).not.toBe("blue");
  expect(
    getComputedStyle(wrapper.querySelector(".current-score:not(:empty)"))
      .borderColor,
  ).not.toBe("red");
});
...
test(`does not apply live styles to match element if options.getMatchElement is provided`, () => {
  ...
  const options = {
    getMatchElement: () => {},
    liveMatchBorderColor: "red",
    liveMatchBgColor: "blue",
  };
  const { wrapper } = init(data, options);
  expect(
    getComputedStyle(wrapper.querySelector(".match-body")).borderColor,
