...
test(`fills .side-scores element with a string returned from getScoresHTML`, () => {
  ...
  expect(
    wrapper.querySelector('.side-wrapper[contestant-id="c1"] .side-scores')
      .textContent,
  ).toBe("123");
});
    ...
    matches: [
      {
        roundIndex: 0,
        order: 0,
        sides: [{ contestantId: "c1", scores: [{ mainScore: "1234" }] }],
      },
    ],
  };
...
test(`inserts HTML returned by getScoresHTML to the DOM`, () => {
  ...
  const { wrapper } = init(data, {
    getScoresHTML: () => `<div class="user-score">1</div>`,
  });
  expect(
    wrapper.querySelector('.side-wrapper[contestant-id="c1"] .user-score')
      .textContent,
  ).toBe("1");
});
...
test(`does not call getScoresHTML() if getMatchElement is provided`, () => {
  ...
  init(data, {
    getScoresHTML,
    getMatchElement: () => document.createElement("div"),
  });
...
test(`calls getScoresHTML even if side.scores is an empty array`, () => {
  const data = {
    rounds: [{}],
    matches: [
      { roundIndex: 0, order: 0, sides: [{ contestantId: "c1", scores: [] }] },
    ],
  };
  const getScoresHTML = jest.fn();
  init(data, { getScoresHTML });
