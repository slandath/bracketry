...
test(`renders as much <player-wrapper>s as a contestant.players.length`, () => {
  ...
  const player_wrappers = wrapper.querySelectorAll(
    '.side-wrapper[contestant-id="c1"] .player-wrapper',
  );
...
test(`renders only 1 entry status and scores per side regardless of players.length`, () => {
  ...
  const { wrapper } = init(data);
  expect(
    wrapper.querySelectorAll('.side-wrapper[contestant-id="c1"] .entry-status')
      .length,
  ).toBe(1);
  expect(
    wrapper.querySelectorAll('.side-wrapper[contestant-id="c1"] .side-scores')
      .length,
  ).toBe(1);
});
...
test(`options.getNationalityHTML() is called for each player`, () => {
  ...
  const getNationalityHTML = jest.fn(
    (n) => `<div class="custom-nationality">${n}</div`,
  );
