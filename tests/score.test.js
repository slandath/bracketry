...
test(`renders a string for mainScore`, () => {
  ...
  expect(
    wrapper.querySelector(".side-own-single-score .main-score").textContent,
  ).toBe("Rt");
});
...
test(`renders empty <side-own-single-score> for an empty side.scores when other side has a scores entry`, () => {
  ...
  expect(
    wrapper.querySelector(
      '.side-wrapper[contestant-id="c2"] .side-own-single-score',
    ).textContent,
  ).toBe("");
});
...
test(`renders scores even if side has no "contestantId"`, () => {
  ...
  const { wrapper } = init(data);
  expect(wrapper.querySelector(".side-own-single-score").textContent).toBe(
    "12",
  );
});
...
test(`renders as much <single-score-wrapper>s as the longest scores length of both sides`, () => {
    ...
    matches: [
      {
        ...
        sides: [
          {
            contestantId: "c1",
            scores: [{ mainScore: "1" }, {}, { mainScore: "3" }],
          },
          {
            contestantId: "c2",
            scores: [{ mainScore: "1" }, { mainScore: "4" }],
          },
        ],
      },
    ],
  ...
  expect(
    wrapper.querySelectorAll(
      '.side-wrapper[contestant-id="c1"] .single-score-wrapper',
    ).length,
  ).toBe(3);
  expect(
    wrapper.querySelectorAll(
      '.side-wrapper[contestant-id="c2"] .single-score-wrapper',
    ).length,
  ).toBe(3);
});
    ...
    matches: [
      {
        roundIndex: 0,
        order: 0,
        sides: [{ scores: [{ mainScore: "6", subscore: 7 }] }],
      },
    ],
  };
  const { wrapper } = init(data);
...
test(`renders .single-score-wrapper for both sides if one of them has mainScore`, () => {
  ...
  const { wrapper } = init(data);
  expect(
    wrapper.querySelector('.side-wrapper[contestant-id="c1"] .main-score'),
  ).not.toBe(null);
  expect(
    wrapper.querySelector('.side-wrapper[contestant-id="c2"] .main-score'),
  ).not.toBe(null);
});
    ...
    matches: [
      {
        roundIndex: 0,
        order: 0,
        sides: [{ scores: [{ mainScore: "6", subscore: Array }] }],
      },
    ],
  };
  const { wrapper } = init(data);
    ...
    matches: [
      {
        roundIndex: 0,
        order: 0,
        sides: [{ scores: [{ mainScore: "6", subscore: NaN }] }],
      },
    ],
  };
  const { wrapper } = init(data);
    ...
    matches: [
      {
        ...
        order: 0,
        sides: [
          { contestantId: "c1", scores: [{ isWinner: true, mainScore: "6" }] },
        ],
      },
    ],
  };
