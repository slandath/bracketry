...
test(`calls getRoundTitleElement for each round`, () => {
  ...
  init({ rounds: [{}, {}, {}, {}, {}, {}] }, { getRoundTitleElement });
...
test(`appends  to the DOM all Elements returned from getRoundTitleElement`, () => {
  ...
  expect(
    wrapper.querySelector(".round-title:first-child .custom-round-title")
      .textContent,
  ).toBe("first");
  expect(
    wrapper.querySelector(".round-title:nth-child(2) .custom-round-title")
      .textContent,
  ).toBe("second");
});
