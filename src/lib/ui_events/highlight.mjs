...
export const update_highlight = (matches_positioner, new_id) => {
  ...
  [
    ...matches_positioner.querySelectorAll(
  ...
  sides_to_highlight.reverse().forEach((s, i) => {
    ...
    if (i === 0)
      s.closest(".match-wrapper").classList.add(last_highlighted_class);

    const last_round_parent = s.closest(".bronze-round-wrapper");
    if (last_round_parent) {
      last_round_parent
        .querySelector(".pseudo-round-wrapper:first-child .match-wrapper")
