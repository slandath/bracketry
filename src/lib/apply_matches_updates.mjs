...
export const apply_matches_updates = (
  updates,
  all_data,
  html_shell,
  get_option,
  ...
  if (!Array.isArray(updates)) {
    console.warn(
      `applyMatchesUpdates must be called with an array of matches, instead called with: `,
      updates,
    );
    return;
  }
  ...
  updates.forEach((u) => {
    ...
    const round_el = html_shell.the_root_element.querySelector(
      `.round-wrapper[round-index="${u.roundIndex}"]`,
    );
    const match_wrapper = round_el?.querySelector(
