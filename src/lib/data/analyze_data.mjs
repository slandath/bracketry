...
export const ananlyze_data = (all_data) => {
  ...
  if (all_data.contestants !== undefined && !is_object(all_data.contestants)) {
    all_errors.push({
      is_critical: true,
      message:
        "data.contestants must be an object which maps ids to contestants, instead got: ",
      data: all_data.contestants,
    });
  }

  is_object(all_data.contestants) &&
    Object.values(all_data.contestants).forEach((contestant) => {
      ...
      if (
        typeof contestant.entryStatus !== "undefined" &&
        typeof contestant.entryStatus !== "string"
      ) {
        all_errors.push({
          is_critical: false,
          message:
            "If entryStatus is provided for a contestant, it must be a string: ",
          data: contestant,
        });
      }
        ...
        contestant.players.forEach((player) => {
          ...
          if (typeof player.title !== "string") {
            all_errors.push({
              is_critical: false,
              message:
                "Player must have a title, and it must be a string, instead got: ",
              data: player,
            });
          }

          if (
            typeof player.nationality !== "undefined" &&
            typeof player.nationality !== "string"
          ) {
            all_errors.push({
              is_critical: false,
              message:
                "If nationality is provided for a player, it must be a string: ",
              data: player,
            });
          }
        });
