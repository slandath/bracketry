...
function try_assign_new_data(old_data: Data, new_data: Data): boolean {
  const { have_critical_error } = handle_data_errors(ananlyze_data(new_data))
  if (have_critical_error)
    return false
...
export function createBracket(initial_user_data: Record<string, unknown>, user_wrapper_el: HTMLElement | null, user_options: Record<string, unknown>): BracketInstance {
  ...
  const uninstall = (): void => {
    ...
    if (instance_i > -1)
      all_bracketry_instances.splice(instance_i, 1)
  ...
  instance = {
    ...
    moveToLastRound: () => {
      if (alive)
        navigation.set_base_round_index(Infinity)
    },
    setBaseRoundIndex: (i: number) => {
      ...
      if (alive)
        navigation.set_base_round_index(i)
    },
