...
const wheelOpt: AddEventListenerOptions | boolean = supportsPassive ? { passive: false } : false;
const wheelEvent =
  "onwheel" in document.createElement("div") ? "wheel" : "mousewheel";

export const restrict_native_scroll = (el: Element, make_scroll_jump: (delta: number) => void) => {
  ...
  el.addEventListener(wheelEvent, deputise_for_wheel as EventListener, wheelOpt); // modern desktop
  el.addEventListener("keydown", preventDefaultForScrollKeys as EventListener, false);

  const release_native_scroll = () => {
    el.removeEventListener("DOMMouseScroll", preventDefault, false);
    el.removeEventListener(wheelEvent, deputise_for_wheel as EventListener, wheelOpt);
    el.removeEventListener("touchmove", preventDefault, wheelOpt);
    el.removeEventListener("keydown", preventDefaultForScrollKeys as EventListener, false);
  };
