...
export default function SelectionTool({ data, onPick}: Props) {
    ...
    const firstMatch = useMemo<Match | null>(() => {
        ...
        a.roundIndex === b.roundIndex ? a.order - b.order : a.roundIndex - b.roundIndex);
    ...
    <div style={{
      ...
      <h4 style={{ marginTop: 0 }}>
        First match — Round {firstMatch.roundIndex + 1} · Order {firstMatch.order}
      </h4>

      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        <TeamCard team={left} onPick={() => handlePick(left?.id)} selected={predictedTeam === left?.id} />
        <div style={{ width: 48, textAlign: "center", color: "#666" }}>vs</div>
        <TeamCard team={right} onPick={() => handlePick(right?.id)} selected={predictedTeam === right?.id} />
      </div>

      <div style={{ marginTop: 12 }}>
        <small style={{ color: "#666" }}>
          Match ID: {firstMatch.id ?? "(none)"} · Status: {firstMatch.matchStatus ?? "n/a"}
        </small>
      </div>
    </div>
  );
...
function TeamCard({
  ...
  return (
    <div style={{
      ...
      {team.logoUrl ? (
        <img src={team.logoUrl} alt={`${team.name} logo`} style={{ width: 56, height: 56, objectFit: "contain" }} />
      ) : (
        <div style={{
          width: 56, height: 56, background: "#f3f3f3", borderRadius: 6,
