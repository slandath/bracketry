...
describe("Vertical scroll on button clicks", () => {
  it("scrolls down on button click", () => {
    cy.visit(
      `http://localhost:3000/cypress?${get_query({
        verticalScrollMode: "buttons",
      })}`,
    );
    cy.get(".button-down").click();
    cy.get(".matches-positioner").should(
      "have.css",
      "transform",
  ...
  it("scrolls down on button click only to the edge of content", () => {
    ...
    cy.get(
      ".round-wrapper[round-index=0] .match-wrapper[match-order=0]",
    ).should("not.be.visible");
    cy.get(
      ".round-wrapper[round-index=0] .match-wrapper[match-order=55]",
    ).should("not.be.visible");
    cy.get(
      ".round-wrapper[round-index=0] .match-wrapper[match-order=60]",
    ).should("be.visible");
    cy.get(
      ".round-wrapper[round-index=0] .match-wrapper[match-order=63]",
    ).should("be.visible");
  });
  ...
  it("translateY (synthetic scroll position) is adjusted after navigation", () => {
    ...
    cy.get(".matches-positioner").should(($p) => {
      const translateY = new DOMMatrix(getComputedStyle($p[0]).transform).f;
